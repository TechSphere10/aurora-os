...
bool install_package(const char *name, const char *version, bool system_package) {
    ...
    kstrcpy(packages[index].name, name);
    kstrcpy(packages[index].version, version);
    ...
    for (uint32_t i = 0; i < sizeof(available_packages)/sizeof(available_packages[0]); i++) {
        if (kstrcmp(available_packages[i], name) == 0) {
            kstrcpy(packages[index].description, package_descriptions[i]);
            break;
        }
    }
    // Add to file system
...
uint32_t list_available_packages(char *buffer, uint32_t buffer_size) {
    ...
    for (uint32_t i = 0; i < sizeof(available_packages)/sizeof(available_packages[0]); i++) {
        ...
        for (uint32_t j = 0; j < package_count; j++) {
            if (kstrcmp(packages[j].name, name) == 0) {
                installed = true;
                break;
            }
        }
        ...
        if (!installed && offset < buffer_size - 64) {
            offset += ksnprintf(buffer + offset, buffer_size - offset, "%s\n", available_packages[i]);
            count++;
        }
    }
